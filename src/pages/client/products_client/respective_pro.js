...
const respective_pro = (similarbooks,id)=>{
    const book = similarbooks.filter((item)=>{
        return item.id != id
    })
 return `  ${book?.map(function (similarbooks) {
        return /*html*/`
                <div class="m-3">
                    <div class="w-[auto] h-[auto] mx-auto p-1">
                        <a href="/products/${similarbooks.id}&cate_id=${similarbooks.categories.id}">
